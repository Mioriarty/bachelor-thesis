...
\begin{definition}
    Let $F$ be a field. Let $S_{\alpha}(F^{m \times n})$ be the set of matricies of size $m \times n$, who's column all sum up to $\alpha \in F$, namely:
    $$S_{\alpha}(F^{m \times n}) := \{A \in F^{m \times n}\mid \forall j \in [n]\colon \sum_{i=1}^{m} A_{ij} = \alpha\}$$
